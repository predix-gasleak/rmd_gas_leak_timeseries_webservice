...
import com.ge.predix.solsvc.api.GasLeakDataApi;
...
import com.ge.predix.timeseries.entity.tags.TagsList;
 ...
 */
@Component
public class GasLeakDataImpl implements GasLeakDataApi {
	...
	private static Logger log = LoggerFactory.getLogger(GasLeakDataImpl.class);

	/**
	 * -
	 */
	public GasLeakDataImpl() {
		super();
	}
	...
	@Override
	public Response getLastHourWindDataPoints(String id, String authorization,String starttime,String taglimit,String tagorder) {
		if (id == null) {
			return null;
		}
	...
	@Override
	public Response getLatestWindDataPointsAll(String authorization) {
		
		List<Header> headers = generateHeaders();

		
		TagsList tagList = getTagList();
		List<String> tags = tagList.getResults();
		DatapointsLatestQuery dpQuery = buildLatestDatapointsQueryRequestAll(tags);
	...
	public TagsList getTagList(){
		...
		TagsList tagList = this.timeseriesFactory
				.listTags(this.timeseriesRestConfig.getBaseUrl(),headers);
		return tagList;
	...
	private DatapointsLatestQuery buildLatestDatapointsQueryRequestAll(List<String> tagsList) {
		DatapointsLatestQuery datapointsLatestQuery = new DatapointsLatestQuery();
		List<com.ge.predix.timeseries.entity.datapoints.queryrequest.latest.Tag> tags = new ArrayList<com.ge.predix.timeseries.entity.datapoints.queryrequest.latest.Tag>();
	...
	private void createMetrics() {
		for (int i = 0; i < 10; i++) {
			...
			Body body = new Body();
			body.setName("gas_leak_simulator"); //$NON-NLS-1$
